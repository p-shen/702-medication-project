...
getReasonsFromFile <- function(row) {
  ...
  return(paste(content.sentences[index], collapse = " "))
}
...
```{r}
multi.classes <- df.output %>% group_by(USPClass) %>% summarise(count=n()) %>% arrange(desc(count)) %>% filter(count>5)
...
inTrain <-  createDataPartition(df.output$USPClass, p = 0.75, list = FALSE)
dfTrain <- df.output[inTrain,]
dfTest <- df.output[-inTrain,]
...
```{r}
library(keras)
train_data <- dfTrain$rsentences
train_labels <- dfTrain$USPClass
test_data <- dfTest$rsentences
test_labels <- dfTest$USPClass

max_words <- 20
epochs <- 10
num_classes <- length(levels(train_labels))
```
...
```{r}
f <- read_file("data_y.txt")
f.str <- strsplit(f, ",")[[1]]
length(levels(factor(f.str)))
```
