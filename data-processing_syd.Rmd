...
```{r}
filepath <- "data/usyd/"
annot <- list.files(path=filepath)
...
for(file in annot){
  ...
  file <- paste0(filepath, file)
  df.this <- read.table(file, sep='|', header = F, stringsAsFactors = F, quote = "")
...
getReasonsFromFile <- function(row) {
  ...
  content <- gsub("\n", " ", content)
  content.sentences <- unlist(strsplit(content, "[.]"))
  content.sentences <- tolower(content.sentences)
  index <- grep(tolower(row["rcontent"]), content.sentences)
  if(length(index) == 0){
    return(NA)
  }
  return(paste(content.sentences[index-2:index+2], collapse = " "))
}
...
df$USPClass[df$mcontent == "colace ( docusate sodium )"] <- "Gastrointestinal Agents"
df$USPClass[df$mcontent == "nitroglycerin 1/150"] <- "Cardiovascular Agents"
df$USPClass[df$mcontent == "ppi"] <- "Gastrointestinal Agents"
df$USPClass[df$mcontent == "ace inhibitor"] <- "Cardiovascular Agents Agents"
df$USPClass[df$mcontent == "levophed"] <- "Blood Products/Modifiers/Volume Expanders"
df$USPClass[df$mcontent == "steroid"] <- "Anti-inflammatory Agents"
df$USPClass[df$mcontent == "milk of magnesia ( magnesium hydroxide )"] <- "Gastrointestinal Agents"
df$USPClass[df$mcontent == "milk of magnesia"] <- "Gastrointestinal Agents"
df$USPClass[df$mcontent == "phenergan ( promethazine hcl )"] <- "Anesthetics"
df$USPClass[df$mcontent == "senna tablets"] <- "Gastrointestinal Agents"
```
...
## select rows with more than  data points
```{r}
...
# write.table(dfTest$rsentences, file="test_x.txt", row.names = F, col.names = F, sep = "\t")
# write.table(dfTest$USPClass, file="test_y.txt", row.names = F, col.names = F, sep = "\t")
```
